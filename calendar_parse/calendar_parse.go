...
func generateHTML(events []gocal.Event, start time.Time, end time.Time) (string, error) {
	...
	preparedEvents := groupEvents(events)
	preparedEvents = addEmptyDaysToEvents(preparedEvents, start, end)
...
func groupEvents(events []gocal.Event) map[int]map[string]map[int]DayData {
